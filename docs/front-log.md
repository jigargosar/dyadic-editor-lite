...
- Items live loosely at top; related ones sit together.
- **Done** (bottom): a requirements doc — each entry states what the app must do. An item enters only once it's genuinely done and tested.
...
- `minimize` handler: native minimize fires _after the fact_ (not cancelable) → `hide()`, routing the OS minimize button to the tray too.
...
- [ ] Check off the now-implemented feature items below once committed (tab title width, save indicator, Tab-in-Ex bug, command palette, placeholder styling, last-tab no-op). Partially done 2026-07-15: only the 2 static-verified items (tab width, placeholder) checked off. The other 4 stay unchecked — ⚠️ code present but unrun (save dot, Tab-in-Ex, last-tab), ❓ presence only / internals unread (command palette). To finish: run the app + read `CommandPalette.tsx`, then check off what passes.

### Feature backlog / bugs

- [x] Tab min/max title size — constrain tab title width (currently `max-w-[180px]` hardcoded in the tab bar) with a sensible min so very short titles ("a", "Untitled") don't look cramped/inconsistent next to long truncated ones. ✅ code-verified (`App.tsx:368`, static CSS — high confidence).
- [ ] Communicate content saved or pending — no visual indicator today that autosave has happened or is queued (300ms debounce is invisible to the user). Needs a small status cue (e.g. dot/label near the tab or window title) reflecting "saved" vs "unsaved changes pending" state. ⚠️ code present & reads correct (`App.tsx:363–365`, `dirtyIds`/`markDirty`) but NOT runtime-verified — haven't watched the dot toggle live. Leave unchecked until run.
- [ ] Bug: Tab key doesn't work in Ex command mode — pressing `Tab` while the `:` command line is open shifts browser-native DOM focus to the next focusable element (the VIM button) instead of being handled inside the command line (e.g. for Ex command completion). Likely the command-line input isn't intercepting/preventDefault-ing Tab before the browser's native tab-order takes over. ⚠️ code present & reads correct (`App.tsx:282`) but NOT runtime-verified. Leave unchecked until run.
- [ ] Consider dropping the bare `:` Ex command interface in favor of a modern command/action picker (Cmd+K-style palette): searchable, shows all available commands and their shortcuts/keybindings at a glance, and easy to register new commands into as the app grows — addresses both the discoverability problem (Ex commands are invisible unless you already know them) and gives a natural home for future actions beyond just tabs. ❓ `CommandPalette.tsx` exists and is imported, but its internals are UNREAD — presence only, not verified complete/correct. (Note: added _alongside_ the `:` line, not a replacement — the palette's own Tab-escape bug is the open item below.) Leave unchecked until internals read + run.
- [x] Placeholder styling — the CodeMirror empty-state placeholder ("Start typing.") currently uses the library default styling; needs a pass to match the app's theme (color/opacity/font) instead of default gray. ✅ code-verified (`App.tsx:211`, `:218`, static — high confidence).
- [ ] Closing the last tab — current behavior always replaces the last tab with a fresh empty "Untitled" tab regardless of content. Change to: if the last remaining tab is already empty, closing it should be a no-op (disallow/do nothing, since there's nothing to lose and nothing gained by replacing empty with empty); if the last tab has content, closing it should still open a fresh empty tab as it does now. ⚠️ code present & reads correct (`App.tsx:121–142`, `content.trim()==='' → return`) but NOT runtime-verified. Leave unchecked until run.
- [ ] Bug: Tab in the Ctrl/Cmd+K command palette escapes the dialog — Tab should move focus to the next item _within_ the palette (focus trap), not shift DOM focus out to the page behind it. Fix by capturing Tab and cycling the selection with the background made `inert`, or by switching the palette to the native `<dialog>` element (which traps focus for free) — whichever works cleanly.
